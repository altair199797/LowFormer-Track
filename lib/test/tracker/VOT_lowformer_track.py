...
class MobileViTv2Track(BaseTracker):
    ...
    def __init__(self, params, dataset_name, test=False, args=None):
        ...
        with open("/home/moritz/Research/SMAT/modelprint.txt","w") as writefile:
            writefile.write(str(network.backbone))
        ...
        if test:
            ...
            print("SOT checkpoint is loaded:",self.params.checkpoint)
            ck = torch.load(self.params.checkpoint, map_location='cpu', weights_only=False)['net']
            # print(ck.keys())
    ...
    def initialize(self, image, info):
        ...
        if isinstance(info, list):
            ...
            info = {"init_bbox": [info.x, info.y, info.width, info.height]}
    ...
    def track(self, image, info: dict = None):
        ...
        ## VOT return
        return self.state, out_dict["max_score"].item()
        """
...
def lowformer_track_imple():
    import importlib
    name = "mobilevitv2_track"
    parameter_name = "mobilevitv2_256_128x1_ep300_lasot_got_coco_lowformit_b15"
    param_module = importlib.import_module('lib.test.parameter.{}'.format(name))
    params = param_module.parameters(parameter_name)
    params.debug = 0
    
    model = MobileViTv2Track( params, None, test=False, args=None)
    print(model)
    return model
    
if __name__ == "__main__":
    lowformer_track_imple()